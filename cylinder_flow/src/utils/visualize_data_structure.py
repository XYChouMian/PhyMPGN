...
def plot_data_structure(h5_file_path: str, show_velocity_background: bool = True):
    """
    绘制H5文件数据的空间结构示意图
    ...
    if show_velocity_background:
        ...
        ax.set_title(f'Data Structure (with Velocity Background) - {file_name}')
    else:
        file_name = Path(h5_file_path).name
        ax.set_title(f'Data Structure (without Background) - {file_name}')

    # 自动定义颜色映射（支持任意数量的节点类型）
    predefined_colors = ['red', 'blue', 'green', 'orange', 'purple', 'cyan', 'magenta', 'yellow', 'brown', 'pink']
    color_map = {}
    for i, node_type_name in enumerate(node_types.keys()):
        color_map[node_type_name] = predefined_colors[i % len(predefined_colors)]
    ...
    for node_type_name, indices in node_types.items():
        if len(indices) > 0:
            ...
            if len(indices) > total_nodes * 0.5:
                print(f"Skipping {node_type_name} (too many nodes: {len(indices)} > {total_nodes * 0.5})")
                continue
...
def visualize_directory(data_dir: str, show_velocity_background: bool = True):
    ...
    for h5_file in h5_files:
        ...
        fig, ax = plot_data_structure(
            str(h5_file), show_velocity_background=show_velocity_background)

        # 保存图片
        output_file = output_dir / f"{h5_file.stem}_structure.png"
