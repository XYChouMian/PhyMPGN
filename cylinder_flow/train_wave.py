...
from src.utils.utils import AverageMeter, WaveNodeType
from src.datasets.dataset import WaveGraphDataset
from src.models.model import WaveModel, MyLoss


def train(model: WaveModel, accelerator: Accelerator, tr_writer: SummaryWriter, val_writer: SummaryWriter, tr_loader, val_loader, epochs, loss_func,
    ...
    for epoch in range(start_epoch, epochs+start_epoch):
        ...
        for batch in tqdm_iterator:
            ...
            graph = batch
            t = target.shape[0]
            U_pred = model.forward(graph, steps=t-1)   # [t, bxn, 1]
            target = torch.index_select(
                target, 1, batch.truth_index)  # [t, n', 1]

            # 波方程专用掩码：只计算内部节点
            mask = (graph.node_type == WaveNodeType.NORMAL)
            ...
            while __state__ != 'END':
                ...
                if torch.any(torch.isnan(tr_batch_loss)):
                    nan_mask = torch.isnan(U_pred)
                    nan_ratio = nan_mask.float().mean().item()

                    if nan_ratio > 0:
                        if nan_ratio > 0.10:
                            raise ValueError(
                                f"NaN ratio too high in U_pred: {nan_ratio:.2%}")

                        # 小于 10%，填充为 0
                        logger.warning(
                            f"Fixing {nan_ratio:.2%} NaNs in U_pred with 0s.")
                        U_pred[nan_mask] = 0.0
                else:
                    __state__ = 'END'
...
def val_evaluate(accelerator: Accelerator, model: WaveModel, val_loader, loss_func, min_val_loss, ckpt_path_val):
    model.eval()
    val_loss = 0
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_loader):
            ...
            gathered_cur_val_loss = accelerator.gather_for_metrics(
                cur_val_loss)
