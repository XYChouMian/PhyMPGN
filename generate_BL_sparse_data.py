...
支持稀疏采样边界条件以减少Dirichlet约束点数量
重要：真实数据和图结构保持密集，只有边界条件进行稀疏采样
"""
...
def create_sparse_dirichlet_indices(height: int, width: int, sparse_rate: int) -> dict:
    """
    创建稀疏采样的Dirichlet边界节点索引（只对内部区域进行稀疏采样）

    只有两个边界条件：
    - exp: 内部区域的稀疏采样点作为"实验数据"边界条件
    - wall: 下边界（第0行），始终为0

    Args:
        height: 网格高度（包含wall行）
        width: 网格宽度
        sparse_rate: 稀疏采样率，例如4表示每4x4区域采样1个点

    Returns:
        sparse_node_type: 包含不同类型节点索引的字典
    """
    n_nodes = height * width

    # wall节点：第0行（y=0），所有列（保持完整，不进行稀疏采样）
    wall_indices = list(range(width))  # 0 to width-1

    # exp节点：内部区域的稀疏采样点（排除wall行）
    exp_indices = []

    # 对内部区域（第1行到最后一行）进行稀疏采样
    ...
    # inner节点：所有非边界节点（包括没有采样的内部点）
    boundary_set = set(wall_indices) | set(exp_indices)
    inner_indices = [i for i in range(n_nodes) if i not in boundary_set]
...
def generate_bl_data_file(
    ...
    with h5py.File(output_file, 'w') as f:
        ...
        if sparse_rate is not None:
            f.attrs['boundary_sparse_rate'] = sparse_rate
...
def generate_grid_and_topology(
    ...
    """
    生成网格坐标、mesh、节点类型等拓扑信息
    **保持密集图结构，只对边界条件进行稀疏采样**
    ...
    Returns:
        pos: 节点位置数组 (n_nodes, 2) - 密集
        mesh: 网格连接数组 (n_edges, 2) - 密集
        node_type: 节点类型字典 - 可能进行了边界条件稀疏采样
        height: 含wall行的总高度
        width: 宽度
        n_nodes: 节点总数
    """
    ...
    height = grid_x.shape[0]  # dense_height + 1（wall行）
    width = grid_x.shape[1]
    n_nodes = height * width
    ...
    if sparse_rate is not None:
        # 对边界条件进行稀疏采样
        node_type = create_sparse_dirichlet_indices(height, width, sparse_rate)
    else:
        # 原始的密集边界条件
...
def generate_bl_data():
    ...
    TEST_SPARSE_RATE = 4               # 测试集边界条件稀疏采样率（每4x4区域采样1个点）
    ...
    with h5py.File(source_file, 'r') as source_h5:
        ...
        pos_tr, mesh_tr, node_type_tr, height_tr, width_tr, n_nodes_tr = generate_grid_and_topology(
            source_h5, TRAIN_X_TRUNCATE, TRAIN_Y_TRUNCATE, TRAIN_SPARSE_RATE
        )
        ...
        print(f"   网格尺寸: {height_tr} x {width_tr} (含wall行，密集图结构)")
        print(f"   节点总数: {n_nodes_tr}")
        if TRAIN_SPARSE_RATE is not None:
            print(f"   边界条件稀疏采样率: {TRAIN_SPARSE_RATE}x{TRAIN_SPARSE_RATE}")
        ...
        pos_te, mesh_te, node_type_te, height_te, width_te, n_nodes_te = generate_grid_and_topology(
            source_h5, TEST_X_TRUNCATE, TEST_Y_TRUNCATE, TEST_SPARSE_RATE
        )
        ...
        print(f"   网格尺寸: {height_te} x {width_te} (含wall行，密集图结构)")
        print(f"   节点总数: {n_nodes_te}")
        if TEST_SPARSE_RATE is not None:
            print(f"   边界条件稀疏采样率: {TEST_SPARSE_RATE}x{TEST_SPARSE_RATE}")
